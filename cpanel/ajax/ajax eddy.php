<?php
	...
	switch(seguridad($_GET["opcion"])){
		...
		$tipo_propiedad=seguridad($_POST['nombre_tp']);//EDDY AGREGO ESTO
			if($mysqli->query("INSERT INTO tipo_propiedad(Propiedad) VALUES('{$tipo_propiedad}')")){//EDDY MODIFICO ESTO 
				echo true;
			}else{
				echo "Error";
			}
		...
		//Regresa el contenido de la tabla tipo_propiedad del archivo panel/tipo_propiedad.php
			$res = $mysqli->query("select * from tipo_propiedad");//EDDY MODIFICO ESTO
				if($res->num_rows > 0){
					$str = "";
					$i = 1;
		...
		$id=seguridad($_POST['id']);//EDDY AGREGO ESTO
			if($mysqli->query("delete from tipo_propiedad where idTipo_propiedad='{$id}';")){//EDDY MODIFICO ESTO
				echo true;
			}else{
				echo "Error";
			}
		...
		$nuevo=seguridad($_POST['nuevo']);//EDDY AGREGO ESTO
		$id=seguridad($_POST['id']);//EDDY AGREGO ESTO
			if($mysqli->query("update tipo_propiedad set Propiedad='{$nuevo}'where idTipo_propiedad='{$id}';")){//EDDY MODIFICO ESTO
			}else{
				echo "Error";
			}
		break;
		...
		$atributo=seguridad($_POST['nombre_attr']);//EDDY AGREGO ESTO
			if($mysqli->query("INSERT INTO atributo(Atributo_propiedad) VALUES('{$atributo}')")){//EDDY MODIFICO ESTO 
				echo true;
			}else{
				echo "Error";
			}
		...
		//Regresa el contenido de la tabla atributo del archivo panel/atributos.php
			$res = $mysqli->query("select * from atributo");//EDDY MODIFICO ESTO 
				if($res->num_rows > 0){
					$str = "";
					$i = 1;
					while ($aux = $res->fetch_assoc()){
		...
		//Borrar registro de la tablas atributo de la base de datos
			$id=seguridad($_POST['id']);//EDDY AGREGO ESTO
			if($mysqli->query("delete from atributo where id_atributo='{$id}';")){//EDDY MODIFICO ESTO
				echo true;
			}else{
				echo "Error";
			}
		...
		$nuevo=seguridad($_POST['nuevo']);//EDDY AGREGO ESTO
		$id=seguridad($_POST['id']);//EDDY AGREGO ESTO
			if($mysqli->query("update atributo set Atributo_propiedad='{$nuevo}'where id_atributo='{$id}';")){//EDDY MODIFICO ESTO 
				echo true;
			}else{
				echo "Error";
			}
